...
**Tests are self-contained — no live data.** Every test must mock external services (databases, APIs, file systems) or use in-memory fixtures. Tests must not depend on `DATABASE_URL`, a running PostgreSQL server, the state of gold tables, or any pre-built external state. A test that calls `execute_df` directly against a real database, or requires `postgres_ready`/`gold_ready` fixtures, is a contract violation — delete it or convert it to mock `execute_df` at the service boundary.

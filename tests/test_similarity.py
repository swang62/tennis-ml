...
def test_lifetime_playstyle_cols_are_playstyle_only():
    ...
    no surface win rates (own block), no reputation, no recent rolling form,
    and no identity/bio metadata."""
    ...
    # Surface rates + exposure counts live in their own calibrated block, and
    # the reputation signals (rank, match count, career win rate) in their own.
    assert SURFACE_BLOCK_COLS == [
        "hard_win_rate",
        "clay_win_rate",
        "grass_win_rate",
        "hard_matches",
    ...
    assert REPUTATION_BLOCK_COLS == ["current_rank", "match_count", "career_win_rate"]
    excluded_names = {
        ...
        "ioc",
        "country",
        # Career/achievement attributes (reputation block, never playstyle).
        "ranking",
        "player_ranking",
        "rank_points",
        "current_rank",

...
def test_latest_completed_monday():
    assert rankings.latest_completed_monday(date(2026, 1, 5)) == date(2025, 12, 29)
    assert rankings.latest_completed_monday(date(2026, 1, 6)) == date(2026, 1, 5)
    assert rankings.latest_completed_monday(date(2026, 1, 11)) == date(2026, 1, 5)


def test_ranking_mondays_after():
    wm = date(2026, 1, 5)
    assert rankings.ranking_mondays_after(wm, date(2026, 1, 26)) == [
        date(2026, 1, 12),
        date(2026, 1, 19),
    ]
...
def test_ranking_mondays_after_empty_when_current():
    wm = date(2026, 1, 19)
    assert rankings.ranking_mondays_after(wm, date(2026, 1, 19)) == []
    assert rankings.ranking_mondays_after(wm, date(2026, 1, 5)) == []
...
def test_extract_rankings_parses_table():
    rows = rankings.extract_rankings_from_html(_MEGA_TABLE_HTML)
    assert len(rows) == 2
...
def test_extract_rankings_raises_on_challenge_page():
    ...
    with pytest.raises(rankings.RankingsParseError, match="no rankings table"):
        rankings.extract_rankings_from_html(html)
...
<a href="/en/players/carlos-alcaraz/a0e2/overview">Carlos Alcaraz</a>
    ...
    with pytest.raises(rankings.RankingsParseError, match="missing rank cell"):
        rankings.extract_rankings_from_html(html)
...
<tr><td class="rank">1</td><td class="player"><a href="/en/players/novak-djokovic/d643/overview">N. Djokovic</a></td></tr>
    ...
    rows = rankings.extract_rankings_from_html(html)
    assert len(rows) == 1
...
def test_translate_keeps_mapped_drops_unmapped():
    ...
    frame, skipped = rankings.translate_rank_rows(rows, rank_map)

...
export function ResultBadge({ won }: { won: boolean }) {
  return (
    <span className={`badge ${won ? "badge-grass" : "badge-clay"}`}>
      {won ? "Won" : "Lost"}
    </span>
  );
}
    ...
    <div className="form-strip" role="img" aria-label={`Recent form: ${label}`}>
      {results.map((r) => {
        ...
        return (
          <span
            key={`${r}-${n}`}
            className={`form-chip ${r === "won" ? "is-win" : "is-loss"}`}
          >
            {r === "won" ? "W" : "L"}
          </span>
        );
...
export function PlayerPicker({
  ...
  const rankedDefaults = players
    ...
    .slice(0, 20);
  const search = loadedSearchFn ?? searchFn;
  const options =
    q.length === 0
      ? rankedDefaults
      : search
  ...
  const showLoading =
    (loading || searchLoading) && (q.length > 0 || options.length === 0);
              ...
              options.map((p, i) => (
                ...
                >
                  <span className="picker-option-player">
                    ...
                    {p.ioc && (
                      <span className="picker-option-country">{p.ioc}</span>
                    )}
                  </span>
                  {p.current_rank != null && (
                    <span className="picker-option-rank num">
                      #{p.current_rank}
                    </span>
                  )}
                </button>

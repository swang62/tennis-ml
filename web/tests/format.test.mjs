...
import { TIER_LABEL, fairOdds, pct, sanitizeErrorMessage } from '../src/lib/format.ts'
...
test('sanitizeErrorMessage removes known ids by whole word', () => {
  assert.equal(sanitizeErrorMessage('Player S0AG not found', ['S0AG']), 'Player not found')
  assert.equal(
...
test('sanitizeErrorMessage escapes regex metacharacters in ids', () => {
  assert.equal(sanitizeErrorMessage('id a.b not found', ['a.b']), 'id not found')
})

...
test('builds a content-hashed payload plus manifest and consumes the raw input', async () => {
  ...
  const { directoryFileName, searchFileName } = await buildPlayerIndex(path.join(dir, 'player-directory.json'), dir)

  // One hashed payload + the manifest remain; the raw directory input is gone.
  assert.deepEqual((await readdir(dir)).sort(), [MANIFEST_NAME, directoryFileName, searchFileName].sort())
  ...
  const payload = JSON.parse(await readFile(path.join(dir, directoryFileName), 'utf8'))
  assert.deepEqual(payload.players, PLAYERS_FIXTURE)

  // The serialized index deserializes and searches with the Home options.
  const searchPayload = JSON.parse(await readFile(path.join(dir, searchFileName), 'utf8'))
...
test('is deterministic and content-hash sensitive', async () => {
  ...
  const dirC = await fixtureDir({ players: [...PLAYERS_FIXTURE, { ...PLAYERS_FIXTURE[0], player_id: '3' }] })
